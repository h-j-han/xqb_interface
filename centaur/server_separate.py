...
class PlayerClient:
    ...
    def __init__(
        ...
        self.round_number_list = [0, 1] #TODO: why only two
        self.question_index = None
        self.question = None
        self.room_id_base = 'room_1'
        self.room_id_and_round = None
        self.deferreds = []
        self.avail_trans_model = avail_trans_model
    ...
    def new_round(self, chosen_round=None):
        ...
        logger.info(f'*********** new round for player {self.player_name} *************')
        ...
        self.sendMessage({'type': MSG_TYPE_NEW_ROUND})
    ...
    def end_of_round(self):
        ...
        def callback(x):
            chosen_round = int(self.response.get('chosen_round', 0))
    ...
    # individualize
    def check_player_response(self, key, value):
        return (
            self.active
            and self.response is not None
            and key in self.response
            and self.response.get(key, None) == value
        )
    #individualize
    ...
    def new_question(self):
        try:
            ...
            def make_callback():
                def f(x):
                    logger.info(f'{self.room_id_base} [new question] player {self.player_name} ready')
                return f

            def make_errback():
                def f(x):
                    logger.info(f'{self.room_id_base} [new question] player {self.player_name} timed out')
                    #TODO: unregister
        ...
        def calllater():
            # for player in self.players.values():
            logger.info(f"{self.room_id_base} player {self.player_name} score {self.score}")
            self.questions_seen.append(self.question.id)
    ...
    def last_chance(self, countdown):
        ...
        #         continue
        if self.check_player_response('type', MSG_TYPE_BUZZING_REQUEST):
            buzzing_ids.append(self.player_id)
    ...
    def _buzzing(self, buzzing_ids, end_of_question):
        ...
        assert len(buzzing_ids) == 1
        buzzing_id = buzzing_ids[0]
        # green_player = self.players[buzzing_id]
        logger.info(f'{self.room_id_base} [buzzing] player {self.player_name} answering')

        self.info_text += NEW_LINE + BADGE_BUZZ
        self.info_text += ' {}: '.format(boldify(self.player_name))
        ...
        self.sendMessage(msg)
        self.buzzed = True
        self.position_buzz = self.position
        self.questions_answered.append(self.question.id)
        ...
        def errback(x):
            logger.info(f'{self.room_id_base} [buzzing] player {self.player_name} answer time out')
    ...
    def _buzzing_after(self, buzzing_id, end_of_question, timed_out):
        try:
            # green_player = self.players[buzzing_id]
            answer = 'TIME_OUT' if timed_out else self.response['text']
    ...
    def stream_next(self):
        ...
        buzzing_ids = []
        # for player_id, player in self.players.items():
        # TODO: what is the polisy for buzzing multiple time? or just one time?
        # if not self.can_buzz(self.question.id):
        #     continue
        if self.check_player_response('type', MSG_TYPE_BUZZING_REQUEST):
            buzzing_ids.append(self.player_id)
    ...
    def _end_of_question(self):
        ...
        try:
            ...
            # for player_id, player in self.players.items():
            player_in_db = self.db.query(Player).get(self.player_id)
            player_in_db.score = self.score
            player_in_db.questions_seen = self.questions_seen
            player_in_db.questions_answered = self.questions_answered
            player_in_db.questions_correct = self.questions_correct
            self.db.commit()
            ...
            record = Record(
                ...
                position_buzz=self.position_buzz,
                guess=self.buzz_info.get('guess', None),
                result=self.buzz_info.get('result', None),
                qb_score=self.buzz_info.get('qb_score', None),
                ew_score=self.buzz_info.get('ew_score', None),
                explanation_config=json.dumps(self.explanation_config),
                mediator_name=self.mediator.__class__.__name__,
            ...
            round_stat = self.db.query(PlayerRoundStat).get((self.player_id, self.room_id_and_round))
            ...
            round_stat.qb_score += self.buzz_info.get('qb_score', 0)
            round_stat.ew_score += self.buzz_info.get('ew_score', 0)
            if 'result' in self.buzz_info:
                round_stat.questions_answered = round_stat.questions_answered + [self.question.id]
                if self.buzz_info['result']:
...
class BroadcastServerFactory(WebSocketServerFactory):
    ...
    def register(self, client):
        if client.peer not in self.socket_to_player:
            new_player = PlayerClient(client, db=self.db, avail_trans_model=self.avail_trans_model)
            ...
            def callback(x):
                try:
                    ...
                    msg = {
                        ...
                        'qid': qid,
                        'player_list': self.player_list,
                        'info_text': 'Nothing to be shown', # self.info_text,
                        'history_entries': [], # self.history_entries,
                        'length': length,
                    ...
                    if new_player.response.get('start_new_round', False):
                        ...
                        if chosen_round != 0:
                            # TODO: weird
                            self.players[player_id].new_round(chosen_round - 1)
                        else:
                            self.players[player_id].new_round()
                    aa=0
                except Exception:
                    traceback.print_exc(file=sys.stdout)
                # end of callback
